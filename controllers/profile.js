...
exports.addExperience = asyncFun(async (req, res, next) => {
	...
	const { body: { title, company, location, from, to, current, description }, user: { _id: userId } } = req;
...
exports.addEducation = asyncFun(async (req, res, next) => {
	...
	const { body: { school, degree, fieldofstudy, from, to, current, description }, user: { _id: userId } } = req;
...
exports.deleteExperience = asyncFun(async (req, res, next) => {
	const { params: { exp_id: expId }, user: { _id: userId } } = req;
	...
	const isExist = profile.experience.find((exp) => exp._id.toString() === expId);
	if (!isExist) {
		error = {
			type: 'onlyMessage',
			statusCode: 404,
	...
	const removeIndex = profile.experience.map((exp) => exp._id.toString()).indexOf(expId);
...
exports.deleteEducation = asyncFun(async (req, res, next) => {
	const { params: { edu_id: eduId }, user: { _id: userId } } = req;
